                ...
                if (textBoxLogin.Text != "" && textBoxPassword.Text != "" && textBoxPasswordConfirmation.Text != "")
                    if (textBoxPassword.Text.Length >= 8 && textBoxLogin.Text.Length >= 3)
                        if (textBoxPassword.Text == textBoxPasswordConfirmation.Text)
                        {
                            ...
                            MessageBox.Show("Вы успешно зарегистрировались", "Eager Beaver", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                        else
                        {
                            MessageBox.Show("Пароли не совпадают. Попробуйте снова", "Eager Beaver", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    else
                        MessageBox.Show("Введенный пароль или логин не соответствует требованиям", "Eager Beaver", MessageBoxButtons.OK, MessageBoxIcon.Error);
                else
                    MessageBox.Show("Заполните все поля", "Eager Beaver", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            else
                MessageBox.Show("Аккаунт с таким логином уже существует. Попробуйте придумать другой", "Eager Beaver", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        }
